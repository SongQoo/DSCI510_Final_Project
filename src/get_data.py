...
# ==========================================================
# Source C: Labor Market (Web Scraping - requests + pandas)
# ==========================================================
# def get_source_c_labor():
#     """
#     [Source C] BLS Data Viewer: Collect Total, Men, and Women Unemployment Rates
#     - Scrapes HTML tables using pandas.read_html for reliability.
#     - Saves each category as separate CSV files.
#     """
...
#                 if 'year' in cols and 'jan' in cols:
#                     target_df = df
#                     break
...
#     - Uses Archive API to collect articles from 2016 to present. (To get api key, register at developer.nytimes.com)
#     - Respects rate limits (6 seconds between requests).
#     - If a 403 error occurs for a future date (no data yet),
#       it stops and saves the collected data so far.
#     """
...
#     # Check current date (to prevent future requests)
#     now = datetime.datetime.now()
#     current_year = now.year
#     current_month = now.month

#     # Loop from 2016 to Current Year
#     for year in range(2016, current_year + 1):
#         for month in range(1, 13):
#             # Stop if future date
#             if year > current_year or (year == current_year and month > current_month):
#                 print(f" -> Reached future date ({year}-{month:02d}). Stopping collection.")
...
#                     # If 403, assume end of archive and break
#                     print(f"\n    [Stop] 403 Forbidden received at {year}-{month:02d}. Assuming end of archive.")
#                     break # Break inner loop (Move to save)
                    
#                 elif resp.status_code == 429:
#                     print("\n    [Wait] Rate limit hit. Sleeping 30s...")
#                     time.sleep(30)
#                     continue 
...
#         # Break outer loop if 403 encountered
...
# def get_source_d_sentiment_recent(api_key, start_date="20250601", end_date="20251201"):
#     """
#     [Source D - Extension] NYT API: Recent 6 Months Data Collection
#     - Uses Article Search API to collect data for the 'recent 6 months' where Archive API is blocked.
#     - Handles pagination (Page 0, 1, 2...)
#     - Respects rate limits (6 seconds between requests).
#     - Saves raw JSON response for further processing in clean_data.py
#     """
#     print(f"\n[Source D-Extension] Fetching Recent Data ({start_date}-{end_date}) via Article Search API...")
    
#     base_url = "https://api.nytimes.com/svc/search/v2/articlesearch.json"
#     all_articles = []
    
#     # Collect up to 10 pages (100 samples per month)
#     MAX_PAGES = 10 
    
#     # Request by month (June to Nov)
#     months = ["20250601", "20250701", "20250801", "20250901", "20251001", "20251101"]
#     end_days = ["20250630", "20250731", "20250831", "20250930", "20251031", "20251130"]
    
#     headers = {"User-Agent": "Mozilla/5.0"}

#     for i, start in enumerate(months):
#         end = end_days[i]
#         print(f" -> Fetching {start[:6]}...", end=" ")
...
#         with open(save_path, 'w', encoding='utf-8') as f:
#             json.dump(all_articles, f, indent=4)
#         print(f"\n -> Saved {len(all_articles)} recent articles to: {save_path}")
